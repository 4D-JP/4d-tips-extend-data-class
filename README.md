...
仮に顧客が`4000`件あれば`4000`回のループ処理の中でセット演算を実行します。ロジックはシンプルですが，`NEXT RECORD` `ADD TO SET`のようにネットワークリクエストを発生させるコマンドを多用することになり，`RELATE MANY` `ORDER BY` `QUERY SELECTION`のようなデータベース処理を何度も実行しなければなりません。そのたびにカレントセレクションの作成・転送・破棄が繰り返されます。もっとスマートな書き方はないのでしょうか。
